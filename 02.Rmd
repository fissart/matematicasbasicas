...
2.  Dados los conjuntos $A=\left\{x\in\mathbb{Z}|\sim[0\leq -2\vee x>3]\right\}$, $B=\left\{x\in\mathbb{N}|\sim[-1<x\leq 3\rightarrow x=5]\right\}$ y $C=\left\{x\in\mathbb{Z}|(x< -2\vee x\geq 2)\rightarrow x>1\right\}$ Hallar el resultado de $(B\cap C)\Delta(A\cap B)$
